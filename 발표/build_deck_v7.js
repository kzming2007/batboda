...
{
  const s = lightSlide();
  // 제목·이름표는 지도와 흰 판을 깐 뒤에 얹는다. 먼저 넣으면 지도에 덮인다.
  const s3Title = true;
  /*
  ...
  */
  /*
    v7 — 사용자가 v5에서 직접 옮긴 배치를 받았다. 지도를 배경 질감이 아니라 **주역**으로
    올렸고, 그래서 읽히는 뜻이 달라진다. 「자료 넷이 있다」가 아니라 「이 실제 한 칸 위에서
    넷을 모은다」가 된다. PNU와 팜맵 ID가 읽히는 것이 크다 — 필지가 우리가 그린 도형이
    아니라 조회한 실물이라는 증거가 된다.

    그 의도를 깎던 넷을 고쳤다.
    1) 지도를 슬라이드보다 크게 잡아 Leaflet 장치(`+ −`, `N`, 저작권)를 화면 밖으로 밀어냈다.
       특히 `N`이 오른쪽 위 근거 이름표와 겹쳐 「스크린샷을 붙였다」로 읽혔다.
    2) 제목 뒤에만 흰 판을 깔았다. 검은 글자를 지도에 바로 얹으면 프로젝터 감마에 따라
       도로·밭 경계에서 묻힌다. 지도를 가리지 않고 글자만 띄운다.
    3) 공중에 떠 있던 `대관령면 85-61전 · 한 필지`를 지웠다. 왼쪽 아래 카드가 같은 지번을
       PNU까지 갖고 있어 증거로 더 세다. 같은 것을 두 번 적을 이유가 없다.
    4) `4년 가까이 벌어져 있습니다`를 제목 판 안으로 넣었다. 이 장의 논점인데 지도에
       묻혀 있었다.
  */
  /*
    지도를 밀어 조작 장치를 화면 밖으로 보내려 했더니 PNU 카드가 잘렸다. `+ −`는 왼쪽 위,
    카드는 왼쪽 아래라 같은 방향이어서 한쪽을 살리면 한쪽이 죽는다. 그리고 카드가 증거다 —
    필지가 조회한 실물이라는 것을 PNU와 팜맵 ID가 말한다.

    그래서 미는 대신 덮는다. 위쪽 흰 띠를 폭 전체로 깔면 `+ −`(왼쪽 위)와 `N`(오른쪽 위)이
    한 번에 가려지고, 제목·논점·근거 이름표가 놓일 바탕도 함께 생긴다.
    캡처 비율(878×494 = 1.777)이 슬라이드 비율(1.778)과 거의 같아 그대로 꽉 찬다.
  */
  /*
    지도를 6% 키워 오른쪽과 아래를 잘라낸다. 오른쪽 아래 `Leaflet | OpenStreetMap` 표기가
    화면 밖으로 나간다. 색 맞춘 네모로 덮으려 했더니 색이 어긋나 네모가 그대로 보였다.
    왼쪽은 자르지 않는다 — PNU 카드가 거기 있고 그것이 이 장의 증거다.
    비율은 그대로 유지한다(가로·세로를 같은 비로 키운다).
  */
  const scale = 1.06;
  s.addImage({
    path: SHOTS + "20260805_C01_대관령85-61전_경계.png",
    x: 0, y: 0, w: W * scale, h: H * scale,
  });
  /*
    위쪽 띠는 불투명하게 둔다. 반투명으로 두니 지도 조작 단추(`+ −`, `N`)가 비쳐
    「스크린샷을 붙였다」로 읽혔다. 이 자리는 글자만 있으면 되고 지도를 볼 이유가 없다.
  */
  s.addShape("rect", {
    x: 0, y: 0, w: W, h: 2.52,
    fill: { color: P.sf }, line: { color: P.sf, width: 0 },
  });
  ...
  const gapX = 0.22;
  const gapY = 0.26;
  const gridX = W - M - (cellW * 2 + gapX);
  sources.forEach(([kind, org, asOf], i) => {
    const x = gridX + (i % 2) * (cellW + gapX);
    const y = 3.02 + Math.floor(i / 2) * (cellH + gapY);
    s.addShape("roundRect", {
      x, y, w: cellW, h: cellH, rectRadius: 0.06,
      fill: { color: P.sf }, line: { color: P.lineStrong, width: 1 },
    });
    s.addText(kind, {
      x: x + 0.16, y: y + 0.26, w: cellW - 0.32, h: 0.46,
      fontFace: F, fontSize: 18, bold: true, align: "center", margin: 0, valign: "top", color: P.ink,
    });
    s.addText(org.replace(" · ", ""), {
      x: x + 0.16, y: y + 0.78, w: cellW - 0.32, h: 0.28,
      fontFace: F, fontSize: 12.5, align: "center", margin: 0, color: P.inkSoft,
    });
    s.addText(asOf, {
      x: x + 0.16, y: y + 1.06, w: cellW - 0.32, h: 0.32,
      fontFace: F, fontSize: 15, bold: true, align: "center", margin: 0, color: P.cyInk,
    });
  });
  if (s3Title) {
    claim(s, "자료가 없는 것이 아니라, 서로 다른 기관·단위에 있어\n내 농지로 이어지지 않습니다", { y: 0.78 });
    s.addText("같은 농지에 대는 자료 넷이 4년 가까이 벌어져 있습니다", {
      x: M, y: 2.0, w: 8.6, h: 0.34,
...
/* ── S5 · 35초 · 규칙과 AI의 경계 + 검증 5단계 ────────────────────────────── */
{
  ...
  captureFrame(s, {
    x: M, y: 2.1, w: 5.02, h: 4.92, shot: "20260805_C07_04_상단_결론과근거",
    tag: "C7 · 04 쉬운 말 보고서 상단",
    spec: "결론 한 줄 + 「판정 …은 공식 기준으로 규칙이 확정했습니다」 병기 문장\n+ 「이렇게 판단한 근거」 3줄까지.",
  });
  /*
    모델 대체 문장도 슬라이드에서 뺐다. 다섯 상자를 키우니 자리가 겹쳤고, 대본도
    이 문장을 질의응답으로 내렸다. 화면에 「다섯 단계를 거친다」만 남긴다.
  */
  /*
  ...
  */
  /*
    ...
    v4는 다섯 단계마다 설명 문장을 붙였다. 35초에 읽히지 않고, 심사위원은 슬라이드를
    읽지 않는다(2차 멘토링). 그래서 제목 다섯만 남기고 설명은 대본으로 옮긴다.
    발표자가 하나씩 짚으면 되고, 화면은 「다섯 단계를 거친다」까지만 말한다.
  ...
  */
  ...
  /*
    회귀 수치와 각주를 슬라이드에서 함께 뺐다(사용자 지시 — 각주 생략).

    둘은 한 묶음이다. 각주 없이 `560 시나리오`만 남으면 현장 정확도로 읽힌다.
    그래서 숫자도 같이 뺀다. 대본이 두 문장을 붙여 말한다 —
    「감점 폭을 위아래로 20퍼센트 흔든 560개 시나리오에서 주 판정 단계는 바뀌지
    않았습니다. 다만 코드 회귀와 산식 민감도 근거이고 현장 정확도 검증은 아닙니다.」
    발표에서 뒤 문장을 빼면 앞 문장이 과장이 된다.
  */
  slideNo(s, 5);
  s.addNotes(
    "[35초]\n" +
...
/* ── S8 · 25초 · 못 하는 것 ───────────────────────────────────────────────── */
{
  ...
  limits.forEach(([label, fact, note], i) => {
    const y = 2.36 + i * 1.14;
    s.addShape("roundRect", {
      x: M, y, w: [4.62, 5.34, 6.10][i], h: 0.98, rectRadius: 0.06,
      fill: { color: P.sf }, line: { color: P.line, width: 1 },
    });
    s.addText(label, {
