...
/* ── S3 · 22초 · 문제의 실제 형태 ─────────────────────────────────────────── */
{
  ...
  */
  /*
    시안 — 지도를 배경으로 깔고 네 칸을 가운데로 모은다(v4는 가운데를 비운 2×2였다).

    지도를 옅게 전면에 깔면 「이 한 필지 위에서 벌어지는 일」이라는 배경이 먼저 깔리고,
    네 칸이 그 위에 얹힌다. 칸 안 글자를 가운데 정렬해 넷이 한 덩이로 읽히게 한다.
    투명도를 높게 두는 것이 핵심이다 — 배경이 진하면 위의 글자 대비가 무너진다.
  */
  s.addImage({
    path: SHOTS + "20260805_C01_대관령85-61전_경계.png",
    x: 2.1, y: 2.6, w: 9.13, h: 4.06, transparency: 88,
  });
  ...
  const gapX = 0.22;
  const gapY = 0.26;
  const gridW = cellW * 2 + gapX;
  const gridX = (W - gridW) / 2; // 화면 가운데. 좌우 여백이 같아 안정감이 생긴다.
  sources.forEach(([kind, org, asOf], i) => {
    const x = gridX + (i % 2) * (cellW + gapX);
    const y = 2.86 + Math.floor(i / 2) * (cellH + gapY);
    s.addShape("roundRect", {
  ...
  s.addText("대관령면 85-61전 · 한 필지", {
    x: (W - 4) / 2, y: 6.3, w: 4, h: 0.32,
    fontFace: F, fontSize: 14, bold: true, align: "center", margin: 0, color: P.inkSoft,
  });
  slideNo(s, 3);
  s.addNotes(
    "[22초]\n" +
