...
export function reportSystemPrompt() {
  return [
    ...
    /*
      02는 판정을 훑는 화면이다. `-습니다`로 맺는 서술문이 세 번 반복되면 값과 값 사이를
      끝까지 읽어야 무엇이 걸리는지 알 수 있다. 심사에서 `서술형 문장이 많다`는 지적을 받은
      자리이기도 하다. 값을 나르는 두 부분은 어미를 걷어내고 값과 상태만 남긴다.

      `함께 확인할 점`은 서술체로 둔다. 못 하는 것을 알리는 자리라 뚝뚝 끊으면 무성의하게
      읽히고, 조건을 담은 문장이 조각으로 쪼개지면 뜻이 흐려진다.

      04 `쉬운 말 보고서`는 세 부분 모두 서술체다. 그쪽은 읽는 화면이고 이름도 그렇다.
    */
    `${REPORT_SECTIONS[0]}과 ${REPORT_SECTIONS[1]}은 짧은 조각으로 끊어 쓴다. 각 조각은 2~3개로 하고 값과 상태만 담아 마침표로 끝낸다.`,
    `그 두 부분에는 \`-습니다\`, \`-입니다\`, \`-합니다\` 같은 맺음말을 쓰지 않는다. 명사나 명사구로 끝낸다.`,
    "예: `조건부 적합. 토양 pH 6.9 기준 안. 밭 적성등급 2급지·배수 약간 양호로 주의.`",
    "이어 주는 말을 넣지 않는다. `또한`, `따라서`, `그러므로`, `상황입니다` 같은 말을 쓰지 않는다.",
    `${REPORT_SECTIONS[2]}은 2~3문장의 서술체로 쓴다. 여기서는 맺음말을 그대로 쓴다.`,
    "행동 지시는 이 세 부분에 넣지 않는다. 예방 행동은 화면의 행동 목록이 따로 보여준다.",
    "전문 용어를 쓸 때는 괄호로 쉬운 설명을 붙인다. 앞 두 부분에서는 괄호 안도 명사구로 짧게 쓴다.",
