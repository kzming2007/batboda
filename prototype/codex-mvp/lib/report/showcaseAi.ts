...
export function showcaseSystemPrompt() {
  return [
    ...
    // 예시에 `밭 적성등급`을 박아 두면 과수 작물에서도 모델이 `밭`이라고 쓸 여지가 생긴다.
    // 등급 이름은 근거 묶음에 적힌 것을 쓰게 하고, 예시는 용도를 특정하지 않는다.
    "전문 용어에는 괄호로 쉬운 설명을 붙인다. 예: 적성등급(그 용도로 쓰기에 얼마나 맞는지 나눈 공식 등급)",
    /*
      풀이가 원래 용어보다 어려우면 풀이가 아니다. 실제로 `pH(수소 이온 농도를 나타내는
      산도 지수)`가 나왔고, 초보 귀농인에게는 `수소 이온 농도`가 `pH`보다 어렵다.
      아는 말로 바꾸는 것이 이 화면의 목적이라 화학 용어를 금지 목록에 넣는다.
    */
    "풀이에 화학·물리 용어를 쓰지 않는다. `수소 이온 농도`, `전기 전도도`처럼 원래 용어보다 어려운 말은 금지한다. pH는 `흙이 산성인지 알칼리성인지 나타내는 값` 정도로 쓴다.",
